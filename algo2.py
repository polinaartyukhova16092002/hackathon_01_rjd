...
class LocomotiveDispatcher:
    ...
    def process_routes(self, routes_df: pd.DataFrame):
        ...
        sorted_routes = routes_df.sort_values('arrival_min')
        ...
        for _, row in sorted_routes.iterrows():
            route = TrainRoute(
                ...
                travel_duration_min=row['arrival_min'] - row['departure_min']
            )
    ...
    def _find_best_locomotive(self, route: TrainRoute) -> Tuple[Optional[Locomotive], float]:
        ...
        for loco in self.free_locomotives:
            if loco.type == route.loco_type:
                ...
                cost = distance * self.cost_per_km[loco.type]
                if cost < min_cost:
                    min_cost = cost
                    best_loco = loco
    ...
    def _record_assignment(self, route: TrainRoute, loco: Locomotive, distance: float):
        ...
        self.assignments.append({
            ...
            'empty_run_cost': distance * self.cost_per_km[loco.type],
            'previous_station': loco.current_station
        })
...
# Загрузка данных
routes_df = pd.read_excel('routes (2).xlsx', sheet_name='Маршруты')
park_df = pd.read_excel('routes (2).xlsx', sheet_name='Парк')
...
def calculate_objective_function(result: pd.DataFrame, cost_per_empty_km: Dict[str, float],
                                 cost_per_min: Dict[str, float], railway_network: RailwayNetwork) -> float:
    """
    ...
    value_ans = 0
    km_check = 0
    # Для каждой группы (каждого локомотива) вычисляем время простоя
    for name, group in grouped:
        if name != None:
            ...
            value_ans += (24 * 60 - bad_times) * cost_per_min[name.split("_")[0]] + total_empty_distance * cost_per_empty_km[name.split("_")[0]]
            km_check += total_empty_distance

    nan_count = results['locomotive'].isna().sum()
    value_ans += nan_count * ERROR_VALUE

    print(value_ans, km_check)
...
calculate_objective_function(results, cost_per_km, cost_per_min, railway_network)
